...
for key,values in frames_all.items():
    ...
    for val in values_updated:
        df = pd.DataFrame()
        df['frameID'] = key
        df['trackerID'] = val.split(',')[0].split(': ')[-1]
        df['coords'] = val.split(': ')[-1].split('(')[-1].split(')')[0]
        df['xmin'] = val.split(': ')[-1].split('(')[-1].split(')')[0].split(',')[0]
        df['ymin'] = val.split(': ')[-1].split('(')[-1].split(')')[0].split(',')[1]
        df['xmax'] = val.split(': ')[-1].split('(')[-1].split(')')[0].split(',')[2]
        df['ymax'] = val.split(': ')[-1].split('(')[-1].split(')')[0].split(',')[3]
